...
type Section = { title: { de: string; en: string }; items: { de: string; en: string }[] };

const serviceSections: Section[] = [
  {
    title: { de: "Service & Wartung", en: "Service & maintenance" },
    items: [
      { de: "Inspektion nach Herstellervorgaben", en: "Inspection per manufacturer guidelines" },
      { de: "Ölwechsel, Filter, Flüssigkeiten", en: "Oil change, filters, fluids" },
      { de: "Vorbereitung für den Saisonwechsel", en: "Season preparation" },
    ],
  },
  {
    title: { de: "Diagnose", en: "Diagnostics" },
    items: [
      { de: "OBD‑Diagnose & Fehlercodes", en: "OBD diagnostics & fault codes" },
      { de: "Probefahrt & Sichtprüfung", en: "Test drive & visual inspection" },
      { de: "Klare Empfehlung mit nächsten Schritten", en: "Clear next steps" },
    ],
  },
  {
    title: { de: "Bremsen & Fahrwerk", en: "Brakes & suspension" },
    items: [
      { de: "Bremsbeläge & Scheiben", en: "Pads & discs" },
      { de: "Stoßdämpfer, Lager, Gelenke", en: "Shocks, bushings, joints" },
      { de: "Check bei Geräuschen/Vibrationen", en: "Noise/vibration checks" },
    ],
  },
  {
    title: { de: "Reifenservice", en: "Tires" },
    items: [
      { de: "Reifenwechsel & Auswuchten", en: "Change & balancing" },
      { de: "Reifendruck & Profilcheck", en: "Pressure & tread check" },
      { de: "Empfehlungen für neue Reifen", en: "Recommendations" },
    ],
  },
  {
    title: { de: "Klima & Elektrik", en: "A/C & electrical" },
    items: [
      { de: "Klimaservice & Desinfektion", en: "A/C service & sanitation" },
      { de: "Batterie, Licht, Starter/Generator", en: "Battery, lights, starter/alternator" },
      { de: "Schnelle Fehlersuche", en: "Quick troubleshooting" },
    ],
  },
];
...
export function MtAvtoHome() {
  ...
  return (
    <main className="pb-10">
        ...
        <div className="site-shell flex items-center justify-between gap-4 py-4 text-white">
          ...
          <div className="flex items-center gap-3">
            <div className="flex rounded-full bg-white/10 p-1 text-xs">
              <button onClick={() => setLang("de")} className={`rounded-full px-3 py-2 font-semibold ${lang === "de" ? "bg-white text-[#181512]" : "text-white/80"}`}>DE</button>
              <button onClick={() => setLang("en")} className={`rounded-full px-3 py-2 font-semibold ${lang === "en" ? "bg-white text-[#181512]" : "text-white/80"}`}>EN</button>
            </div>
            <a href={telHref} className="rounded-full bg-white px-5 py-3 text-sm font-semibold text-[#181512] transition-transform hover:-translate-y-0.5">
              {t(lang, "cta_call")}
            </a>
          </div>
        </div>
      ...
      <section className="site-shell pt-6 sm:pt-10">
        <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="relative overflow-hidden rounded-[36px] bg-[#171412] text-white">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(188,45,35,0.42),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(255,255,255,0.10),transparent_24%)]" />
            <div className="relative grid gap-8 p-6 sm:p-10">
              ...
              <div className="max-w-3xl">
                ...
                <h1 className="mt-4 text-4xl font-semibold leading-[0.95] tracking-[-0.06em] sm:text-6xl lg:text-[5.2rem]">
                  {t(lang, "hero_title")}
                  <span className="block text-[var(--site-gold)]">{t(lang, "hero_subtitle")}</span>
                </h1>
                <p className="mt-6 max-w-2xl text-base leading-7 text-white/75 sm:text-lg">{t(lang, "hero_lead")}</p>
              </div>

              <div className="flex flex-wrap gap-3">
                <a href={telHref} className="inline-flex rounded-full bg-[var(--site-red)] px-6 py-3.5 text-sm font-semibold text-white transition-transform hover:-translate-y-0.5">
                  {t(lang, "cta_call")}
                </a>
              ...
              <div className="grid gap-3 text-sm text-white/80 sm:grid-cols-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-white/65">{ADDRESS}</div>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <ShieldBadgeIcon className="mt-0.5 h-5 w-5 text-[var(--site-gold)]" />
                  <div>
                    ...
                    <a className="text-white/65 hover:text-white" href={telHref}>{PHONE}</a>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <ShieldBadgeIcon className="mt-0.5 h-5 w-5 text-[var(--site-gold)]" />
              ...
              <div className="flex flex-wrap gap-2">
                {brands.map((b) => (
                  <span key={b} className="rounded-full border border-white/10 bg-white/6 px-4 py-2 text-xs font-semibold text-white/85">{b}</span>
                ))}
              </div>
            </div>
          </div>
      ...
      <section id="services" className="site-shell mt-18">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          {serviceSections.map((sec) => (
            <article key={sec.title.de} className="site-card p-7">
              <h3 className="text-2xl font-semibold tracking-[-0.04em]">{lang === "de" ? sec.title.de : sec.title.en}</h3>
              <div className="mt-5 grid gap-3">
                {sec.items.map((it) => (
                  <div key={it.de} className="flex items-start gap-3 rounded-[22px] border border-[var(--site-outline)] bg-white/70 px-5 py-4">
                    <ShieldBadgeIcon className="mt-0.5 h-5 w-5 text-[var(--site-red)]" />
                    <div className="text-sm leading-6 text-foreground">{lang === "de" ? it.de : it.en}</div>
                  </div>
                ))}
              </div>
            </article>
      ...
      <section id="reviews" className="site-shell mt-18">
        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-3">
          {[0, 1, 2].map((i) => (
            <article key={i} className="site-card p-7">
      ...
      <section id="contact" className="site-shell mt-18">
        <div className="grid gap-5 lg:grid-cols-[0.82fr_1.18fr]">
          <div className="rounded-[36px] bg-[var(--site-red)] p-8 text-white">
            <p className="text-xs uppercase tracking-[0.28em] text-white/72">{t(lang, "contact_title")}</p>
            <h2 className="mt-4 text-4xl font-semibold leading-none tracking-[-0.05em]">{lang === "de" ? "Jetzt anrufen oder Route starten" : "Call now or get directions"}</h2>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href={DIRECTIONS_URL} target="_blank" rel="noreferrer" className="inline-flex rounded-full bg-[#171412] px-6 py-3.5 text-sm font-semibold text-white">{t(lang, "cta_route")}</a>
            </div>
          </div>

          <div className="site-card overflow-hidden">
            <div className="grid min-h-[420px] lg:grid-cols-[0.7fr_1.3fr]">
              <div className="flex flex-col justify-between bg-[#171412] p-7 text-white">
                <div>
                ...
                <a href={DIRECTIONS_URL} target="_blank" rel="noreferrer" className="inline-flex w-fit rounded-full bg-white px-6 py-3.5 text-sm font-semibold text-[#171412] transition-transform hover:-translate-y-0.5">
                  {t(lang, "cta_route")}
                </a>
              </div>

              <div className="relative overflow-hidden bg-[linear-gradient(135deg,#f1e6d6_0%,#fff_100%)] p-7">
                ...
                <div className="relative h-full min-h-[280px] rounded-[28px] border border-white/60 bg-white/72 p-5 shadow-[0_18px_50px_rgba(23,20,18,0.08)] backdrop-blur">
                  ...
                  <div className="mt-8 space-y-4">
                    {[t(lang, "phone"), t(lang, "address"), t(lang, "hours")].map((item) => (
                      <div key={item} className="rounded-2xl border border-[var(--site-outline)] bg-white/80 px-4 py-3 text-sm text-[#403830]">{item}</div>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <footer className="site-shell mt-14 border-t border-[var(--site-outline)] py-10 text-sm text-muted-foreground">
        {t(lang, "footer_note")} • <a className="text-[var(--site-red)]" href={MAPS_URL} target="_blank" rel="noreferrer">Google Maps</a>
      </footer>
    </main>
  );
}
